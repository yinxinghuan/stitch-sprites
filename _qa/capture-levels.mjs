...
async function capture(level, viewport) {
  ...
  if (!geometry.title?.startsWith(`${level} ·`)) throw new Error(`Level ${level} title mismatch: ${JSON.stringify(geometry)}`)
...
if (!requestedLevel) results.push(await capture(35, { width: 320, height: 568 }))
await browser.close()
